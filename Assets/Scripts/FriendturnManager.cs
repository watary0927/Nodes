...
public class FriendturnManager : PunBehaviour, IPunTurnManagerCallbacks
{
    ...
    //private float[] vectorI = { -1.37f, -0.31f, 0.75f, 1.81f, 2.87f };
    // private float[] vectorJ = { -2.15f, -1.09f, -5f, 1.03f, 2.09f };
    ...
    bool localshrink = false;
    bool remoteshrink = false;
    ...
    public void OnTurnCompleted(int obj)//4
    {
        ...
        Debug.Log("localSelection_first:" + localSelection + "remoteSelection_first:" + remoteSelection);
        turnonce = false;
        //if (PhotonNetwork.isMasterClient) StartCoroutine(PrepareForInvoke(L, R, this.turnManager.Turn));*/
    ...
    IEnumerator Placestone(int lo, int re, int nowturn)//1秒間持続させてからdestroy
    {
        ...
        if (lo != re)
        {
            if ((nowturn % 2) == 1 && PhotonNetwork.player.ID == 1 && turnonceEmu)
            {
                Debug.Log("攻撃側かつ1p");
                if (re != 100){
            ...
            else if ((nowturn % 2) == 0 && PhotonNetwork.player.ID == 1 && turnonceEmu)
            {//防御側かつ1p
                Debug.Log("防御側かつ1p");
                if (lo != 100){
                    audioSource.PlayOneShot(Erase);
            ...
            else if ((nowturn % 2) == 0 && PhotonNetwork.player.ID == 2 && turnonceEmu)
            {
                Debug.Log("攻撃側かつ2p");
                if (re != 100) {
                    audioSource.PlayOneShot(Erase);
            ...
            else if ((nowturn % 2) == 1 && PhotonNetwork.player.ID == 2 && turnonceEmu)
            {
                //防御側2p
                Debug.Log("防御側かつ2p");
                if (lo != 100){
        ...
        yield return new WaitForSeconds(0.2f);//もう少し長くてもいいかも
    ...
    public void StartTurn()//ターン開始メソッド（シーン開始時にRPCから呼ばれるようにしてあります。）
    {
    ...
    int Rotate()
    {
        ...
        int rotate = 0;
        if (Math.Abs(dir.z) >= Math.Abs(dir.y) * 1.5f) return 2;
    ...
    private IEnumerator Gravitytime(int[] transboard, int granum)//3秒開けてgravityに仕事をさせる
    {
        ...
        /*foreach (KeyValuePair<int, int> pair in changestones)
        {//帳尻合わせ
            Debug.Log("changestones.Key= " + pair.Key+ "changestones.Value= " + pair.Value);
        }
        foreach (KeyValuePair<int, GameObject> pair in stoneplace)
        {//帳尻合わせ
            Debug.Log("stoneplace.Key= " + pair.Key);
        }*/
        foreach (KeyValuePair<int, int> pair in changestones)
        {//帳尻合わせ
            Vector3 endposition = new Vector3(vectorJ[pair.Value % 5], vectorI[pair.Value / 5], 0f);
        ...
        Debug.Log("Stoneplace.size_second= " + stoneplace.Count);
        Debug.Log("changestones.size_second= " + changestones.Count);
        int[,] newboard = new int[5, 5];
    ...
    {
        if (PlayerPrefs.GetInt("Roomcreate")==1){
            Debug.Log("Roomcreate=1");
            ...
            Resultback.SetActive(true);
            Ratingchange.text = roomname;
        }
        else{
            Debug.Log("Roomcreate=0");
            Resultback.SetActive(true);
    ...
    public override void OnPhotonPlayerDisconnected(PhotonPlayer otherPlayer)
    {
        ...
        else if(resultonce)Endofthegame(JudgeResult());
    }
